...
class FLPostGridModule extends FLBuilderModule {
	...
	public function render_featured_image( $position = 'above' ) {
		...
		if ( has_post_thumbnail() && $settings->show_image && $settings->custom_post_filed !== "staff-presonale" && $settings->custom_post_filed !== "progetti-studenti" ) {
			...
			<div class="fl-post-<?php echo $layout; ?>-image">
				...
				<a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php the_title_attribute(); ?>" class="<?php echo $settings->custom_post_filed ?>" >
					<?php
					// the_post_thumbnail( $settings->image_size ); 
					$render = true;
					//$attachment_id_overlay = get_field('foto_persona_staff',$post_id);
                    //$size_overlay = "milo-hard"; // (thumbnail, medium, large, full or custom size)
                    //$image_overlay = wp_get_attachment_image_src( $attachment_id_overlay,"full" );
                   if ($settings->custom_post_filed == "staff-personale"){
                   	 echo '<img src="'.get_field('foto_persona_staff',$post_id).'" alt="immagine staff"  class="foto-staff">';
                    //echo $image_overlay."sadassa";
                    }
                    if ($settings->custom_post_filed == "progetti-studenti"){
                   	 $attachment_id_overlay = get_field('progetto_immagine',$post_id);
                     $size_overlay = "homepage-hard"; // (thumbnail, medium, large, full or custom size)
                     $image_overlay = wp_get_attachment_image_src( $attachment_id_overlay,$size_overlay );
                   	 echo '<img src="'.$image_overlay[0].'" alt="immagine staff"  class="foto-progetti-studenti">';
                    //echo $image_overlay."sadassa";
                    }
	...
	'layout'        => array(
		...
		'sections'      => array(
			...
			'posts'         => array(
				...
				'fields'        => array(
					'custom_post_filed' => array(
					    'type'          => 'select',
					    'label'         => __( 'Custom Post Style', 'fl-builder' ),
					    'default'       => 'default',
					    'options'       => array(
					        'default'      => __( 'Default', 'fl-builder' ),
					        'staff-personale'      => __( 'Staff/Personale', 'fl-builder' ),
					    	'progetti-studenti'      => __( 'Progetti Studenti', 'fl-builder' ),
