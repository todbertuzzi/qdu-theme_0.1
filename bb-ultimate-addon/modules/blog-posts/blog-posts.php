...
class BlogPostsModule extends FLBuilderModule {
    ...
    public function __construct() {
        ...
        parent::__construct(array(
            'name'          => __('Advanced PostsQ', 'uabb'),
            'description'   => __('Advanced PostsQ', 'uabb'),
    ...
    /** TOD METHOD
    // https://gdswebdesign.co.uk/blog/coding/dynamic-excerpt-lengths-in-wordpress/

    */

    public function gds_dynamic_excerpt( $title_chars_per_line, $excerpt_chars_per_line, $excerpt_length,$obj ) {
        //get length of title
        ...
        $content = strip_shortcodes($obj->post_content);
    ...
    protected function render_image_url( $i, $post_attachment_id ) {
        ...
        if( $id != -1 && $id != "" ) {
            if( $size != 'custom' ) {
                ...
                if($this->settings->styleContenuto == 'news'){
                    $temp = wp_get_attachment_image_src( $id, 'milo-hard' );
                    $image = $temp[0];
                }else{
                    $temp = wp_get_attachment_image_src( $id, $size );
                    $image = $temp[0];
              ...
              //   $image = the_post_thumbnail( 'milo-hard' );
            } else {
            ...
            <?php do_action( 'uabb_blog_posts_before_image', $obj->ID ); ?>
                ...
                <img <?php echo $img_url; ?> alt="<?php echo $img_data['alt']; ?>"  class="img-icon-under" />
                 ...
                 if( isset( $this->settings->show_custom_field ) && $this->settings->show_custom_field != 'none' ) {        
                        ...
                        if ( ! is_array( $field_value ) ) {
                           if($this->settings->styleContenuto == 'news' ){  
                             wp_get_attachment_image_src( $id, 'milo-hard' );
                             $attachment_id_overlay = get_field('icon-overlay',$post_id);
                             $size_overlay = "milo-hard"; // (thumbnail, medium, large, full or custom size)
                             $image_overlay = wp_get_attachment_image_src( $attachment_id_overlay, $size_overlay );
                             echo '<img src='.$image_overlay[0] .' alt="'.$field_value.'"  class="img-icon-overlay">';
        ...
        ?>
            ...
            <div class="row">
                ...
                <?php
                    ...
                    $thelength = 35;
                    $shortTitle = substr($thetitle, 0, $thelength);
                   // echo substr($thetitle, 0, $thelength);
                    if ($getlength > $thelength) $shortTitle =$shortTitle."...";
                    $shortTitle = wp_trim_words ( get_the_title(), 8, ' ...' );
                ?>

                <div class="col-sm-10">
                    <?php
                    $title = '<a href='. apply_filters( "uabb_blog_posts_link", get_permalink( $obj->ID ), $obj->ID ) .' title=' . the_title_attribute('echo=0') . ' tabindex="0" class="">'.  $thetitle .'</a>';
                    //echo $this->settings->styleContenuto;
                    echo apply_filters( 'uabb_advanced_post_title_link', $title, $shortTitle, get_permalink( $obj->ID ), $obj->ID );
                    ?>
                </div>
            </div>
                <?php
    ...
    protected function render_content_section( $obj ) {
        ...
        if( $show_excerpt == 'yes' ) {
            ...
            $contentShort = wp_trim_words ( $obj->post_content, 25, ' ...' );
             
            $contentShort= $this->gds_dynamic_excerpt(15,24,250,$obj);
            ...
            if( $content_count != 0 ) {
                ...
                <?php
                    ...
                    <div class="uabb-blog-posts-description uabb-blog-post-section uabb-text-editor">
                        <?php echo apply_filters('uabb_blog_posts_excerpt',$contentShort); ?>
                    </div>
                <?php
                }
            }
            ...
            foreach( $layout_sequence as $sq ) {
                switch ( $sq ) {
                    ...
                    case 'img' :
                        ...
                        if( substr( $this->settings->layout_sort_order, 0, 3 ) != 'img' && substr( $this->settings->layout_sort_order, -3 ) != 'img' ) {
                           // TOD
                           if(!get_field("video_featured")){
                                echo $this->render_featured_image( 'top', $obj, $i );
                                    
                            }else{
                                echo '<div class="embed-container">';
                                    the_field('video_featured'); 
                                echo '</div>';
                            }
                        }
                        break;
                ...
                'fields'        => array(
                    'styleContenuto'     => array(
                        ...
                        'options'       => array(
                            ...
                            'news'      => __('News Homepage', 'uabb'),
                        )
                    )
                )
            ),
            'navigation-custom-style'  => array(
                'title'         => __( 'Navigation Style', 'uabb' ),
                'fields'        => array(
